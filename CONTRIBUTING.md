...
On first run, `vm-create.sh` generates a throwaway SSH keypair (`ssh-keygen -t ed25519`, no passphrase) at `tests/.vm-key[.pub]` and injects the public key into the VM. It's git-ignored; set `NTFY_TEST_SSH_KEY` to use your own key instead.

#### Deploy to Dev VM

```bash
NTFY_DEV_KEY=tests/.vm-key NTFY_DEV_VM=ntfy-test ./tests/vm-sync.sh
```
...
- `NTFY_DEV_KEY` - **Required**: Path to your SSH private key for the VM. Use `tests/.vm-key` (and `tests/.vm-key.pub`) generated by `vm-create.sh`
- `NTFY_DEV_VM` - VM name (default: `ntfy-dev`)
- `NTFY_DEV_USER` - SSH user on VM (default: `tester`)

The throwaway VM from `vm-create.sh` already contains the public key in `~/.ssh/authorized_keys` and passwordless sudo, so no additional VM-side setup is needed. Ensure the VM is running (`virsh start ntfy-test`).
...
Requires the `mutter-dev-bin` package (`/usr/libexec/mutter-devkit`) for the nested shell window to display:
