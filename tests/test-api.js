...
const SELF_SIGNED = ["1", "true", "yes"].includes(
  (GLib.getenv("NTFY_TEST_SELF_SIGNED") || "").toLowerCase(),
);
...
async function fetchId(message) {
  const { text } = await send(
    makeMsg("GET", `/${TOPIC}/json?poll=1&since=all`),
  );
  return text
    .trim()
    .split("\n")
    .map((l) => JSON.parse(l))
    .find((m) => m.message === message)?.id;
}
...
function subscribe(events) {
  ...
  return api.subscribe(
    TOPIC,
    (e) => events.push(e),
    (e) => events.push({ event: "__error", err: String(e) }),
  );
}
...
async function main() {
  ...
  assert(
    await waitFor(
      () => events.filter((e) => e.event === "message").length >= 2,
    ),
  ...
  assert(
    await waitFor(() =>
      events.some((e) => e.event === "message" && e.message === "C"),
    ),
    "resume: new message C delivered",
  );
  assert(
    events.filter((e) => e.event === "message" && e.message === "A").length ===
      1,
  ...
  sub.cancel();
  assert(
    !events.some((e) => e.event === "message" && e.id === xId),
    "deleted message never delivered as message (batch suppression)",
  );
  assert(
    events.some((e) => e.event === "message_delete" && e.sequence_id === xId),
  ...
  sub.cancel();
  assert(
    !events.some((e) => e.event === "message" && e.id === yId),
    "cleared message never delivered as message (batch suppression)",
  );
  assert(
    events.some((e) => e.event === "message_clear" && e.sequence_id === yId),
  ...
  assert(
    !events.some(
      (e) => e.event === "message" && (e.id === p1Id || e.id === p2Id),
    ),
    "interleaved: neither P1 nor P2 delivered as message",
  );
  assert(
    events.some((e) => e.event === "message_clear" && e.sequence_id === p1Id) &&
      events.some(
        (e) => e.event === "message_delete" && e.sequence_id === p2Id,
      ),
  ...
  await api.publish(
    ...
    `/${TOPIC}`,
    "text/plain",
    new TextEncoder().encode("from publish()"),
    { Title: "t1" },
  );
  events = [];
  sub = subscribe(events);
  ...
  if (SELF_SIGNED) {
    ...
    await api.publish(
      "PUT",
      `/${TOPIC}?filename=t.txt&message=att`,
      null,
    ...
    assert(
      await waitFor(() =>
        events.some(
          (e) => e.event === "message" && e.attachment?.name === "t.txt",
        ),
      ),
