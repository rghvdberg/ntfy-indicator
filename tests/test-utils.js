...
function expectParse(input, baseUrl, topic, name) {
  ...
  assert(
    r.baseUrl === baseUrl && r.topic === topic,
...
async function main() {
  ...
  assert(
    getApiKey(mockSettings('{"https://srv":"k1"}'), "https://srv") === "k1",
    "getApiKey hit",
  );
  assert(
    getApiKey(mockSettings('{"https://srv":"k1"}'), "https://other") === null,
    "getApiKey miss",
  );
  assert(
    getApiKey(mockSettings("not json"), "https://srv") === null,
    "getApiKey malformed",
  );
  assert(
    getApiKey(
