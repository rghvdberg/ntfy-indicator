...
const na = (id, time = 0) => ({
  ...n(id, time),
  attachment: { name: "att.txt", url: "https://x/att.txt" },
});

async function main() {
  // add / dedupe
  assert(
    (await store.addNotification("t-add", n("a1"), 100)) === true,
    "add new message",
  );
  assert(
    (await store.addNotification("t-add", n("a1"), 100)) === false,
  ...
  assert(
    (await store.getUnreadCount("t-add")) === 0,
    "unread 0 after markRead",
  );
  assert(
    (await store.markRead("t-add", "nope")) === false,
    "markRead missing id -> false, no-op",
  );

  // deleteNotification: removes row, records seenIds (even for unknown ids)
  assert(
    (await store.deleteNotification("t-del", "ghost")) === true,
    "delete unknown id still records seenIds",
  );
  let data = await store._readData("t-del");
  assert(data.seenIds.includes("ghost"), "ghost id in seenIds");
  assert(
    (await store.addNotification("t-del", n("ghost"), 100)) === false,
    "seenIds suppresses re-add",
  );
  await store.addNotification("t-del", n("d1"), 100);
  assert(
    (await store.deleteNotification("t-del", "d1")) === true,
    "delete existing row",
  );
  assert((await store.load("t-del")).length === 0, "row removed");
  assert(
    (await store._readData("t-del")).seenIds.includes("d1"),
  ...
  assert(
    rows.every((r) => r.new === false),
    "markAllRead clears new flags",
  );
  assert(
    (await store.getUnreadCount("t-all")) === 0,
    "unread 0 after markAllRead",
  );
  ...
  assert(
    (await store.getUnreadCount("t-count")) === 2,
    "unread counts new rows",
  );
  ...
  assert(
    (await store.load("t-burst")).length === 20,
    "burst: 20 rows, no lost writes",
  );
  ...
  assert(
    (await store2.load("t-burst")).length === 20,
    "second instance reads same data",
  );
  assert(
    (await store2.getLastMessageId("t-wm")) === "id123",
    "second instance reads watermark",
  );
  ...
  assert(
    !Gio.File.new_for_path(dataDir).query_exists(null),
    "data dir removed",
  );
  assert(
    (await store.addNotification("t-heal", n("h1"), 100)) === true,
    "add after dir removal",
  );
  assert(
    Gio.File.new_for_path(getNotificationFile("t-heal")).query_exists(null),
  ...
  assert(
    (await store.load("hl-b100")).length === 100,
    "limit 100: exactly 100 stored",
  );
  ...
  assert(
    rows.length === 100,
    `limit 100: 101 trims to 100 (got ${rows.length})`,
  );
  assert(!rows.some((r) => r.id === "c1"), "limit 100: oldest (c1) dropped");
  assert(
    rows.some((r) => r.id === "c101"),
    "limit 100: newest (c101) kept",
  );
  const t101 = rows.map((r) => r.time);
  assert(
    t101.every((v, i) => i === 0 || t101[i - 1] >= v),
  ...
  assert(
    rows.length === 100,
    `limit 100: 150 trims to 100 (got ${rows.length})`,
  );
  assert(
    !rows.some((r) => r.id === "d1"),
    "limit 100: oldest (d1) dropped on 150",
  );
  ...
  assert(
    rows.length === 10,
    `shrink: 100@100 + 1@10 -> 10 (got ${rows.length})`,
  );
  assert(
    rows.every((r) => r.time >= 92),
    `shrink: keeps newest 10 (times >=92, got ${rows.map((r) => r.time)})`,
  );
  ...
  assert(
    (await store.load("hl-delcnt")).length === 5,
    "delete-slot: 10 @5 -> 5 kept",
  );
  await store.deleteNotification("hl-delcnt", "x6x6"); // splice, no trim -> 4
  assert(
    (await store.load("hl-delcnt")).length === 4,
    "delete-slot: delete -> 4 (no trim)",
  );
  ...
  assert(
    !rows.some((r) => r.id === "x6x6"),
    "delete-slot: deleted id not occupying a slot",
  );
  ...
  assert(
    rf && rf.new === false,
    "readflag: read status preserved on kept message after trim",
  );
  ...
  assert(
    rows.every((r) => r.time >= 41),
    `burst: keeps newest 10 (times >=41)`,
  );
  ...
  assert(
    rows.map((r) => r.id).join(",") === "f1,f2",
    `tie: stable FIFO keeps first-inserted (got ${rows.map((r) => r.id)})`,
  );
  ...
  assert(
    rows.find((r) => r.id === "m91").new === false,
    "markover: marked read",
  );
